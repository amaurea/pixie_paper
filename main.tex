...
\usepackage{amsmath,gensymb,color,graphicx,array,authblk}
...
has a 1-dimensional toy example which demonstrates this effect.
...
However, that would not have been a good thing, because while it's
possible
to make the simulator match the map-makers fourier-sub-pixels,
the real data won't be as kind. Of course, reality is just as unlikely
to be fourier-sub-pixel as bicubic-sub-pixel, but the mismatch between
these two gives a good indication of the mismatch one can expect
between the real data and the sub-pixel assumptions made in the map-maker.

\subsection{Instrumental effects}
\subsubsection{Intensity to polarizaiton leakage}
...
for a highly elliptical beam with an elongation of 1.
...
%Intensity to polarization (I-to-P) leakage is a common systematic
%in CMB experiments, but PIXIE's design makes it very robust to
%this effect.
%\begin{itemize}
%	\item I-to-P leakage is rejected because it is not modulated
%		with the telescope's spin
%	\item The two detectors in each horn see
%\end{itemize}
%
%Firstly, since the telescope measures polarization
%via the modulation from its spin, any I-to-P leakage in its optics
%
%
%\subsubsection{Barrel collimation offset}

\subsubsection{Mirror jitter}
\begin{figure}
	\centering
