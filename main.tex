...
filter. We ignore this component in the mapmaker, and simply treat it as
a part of the noise.
...
the mapmaker itself can ignore it, and set $\vec s_\textrm{sky,B}=0$.
...
so we can again set $\vec s_\textrm{sky,B}=0$ as long as we multiply the
...
\begin{align}
	\vec s_\textrm{det}^\textrm{ring}(i) &= G \cdot
		R_\textrm{stok}\Big(\frac{4\pi i}{N_\textrm{spin}}\Big) \cdot
		\vec s_\textrm{sky}\Big(\Big[l=l_\textrm{ring},b=\frac{2\pi i}{N_\textrm{scan}}\Big],
		A_\textrm{delay}\textrm{triangle}\Big(\frac{i}{N_\textrm{stroke}}\Big)\Big)
\end{align}
where $N_\textrm{scan} = 5\,898\,240$,$N_\textrm{spin} = 15\,360$ and
$N_\textrm{stroke} = 1\,920$ are the number
of samples per scan, spin and stroke respectively.
...
traces out a slightly skewed grid (A) instead of the orthogonal grid
that would be convenient (E). But as long as our signal is well-sampled
...
if periodic each of these evenly sampled ring subsets are also peridic, so we
can interpolate via Fourier shifting.

Given an array $a$ of samples of length $N$ and a discrete Fourier transform $F$,
...
\end{align}
which defines the Fourier-shift matrix $\Psi(\Delta)$. Using this, the
scan drift corrected ring TOD $d'$ in terms of the raw TOD $d$ is
\begin{align}
	d'^1_{p_1g_1} &= \Psi\big(g_1/N_\textrm{spin}\big)_{p_1p_1'} d_{p_1'g_1} & i &= p_1N_\textrm{spin} + g_1
\end{align}
...
\begin{align}
	d'^2_{p_2g_2} &= \Psi\big(2g_2/N_\textrm{spin}\big)_{p_2p_2'} d_{p_2'g_2} & i &= p_2N_\textrm{spin}/2 + g_2
\end{align}

Undoing the effect of PIXIE's spin during the mirror motion follows the same
logic on shorter time scales. We now into groups per mirror position (panel D)
such that
\begin{align}
	d^\textrm{ort}_{sd} &= \Psi\big(d/N_\textrm{stroke}\big)_{ss'} d'_{s'd} & i &= sN_\textrm{stroke} + d
\end{align}
